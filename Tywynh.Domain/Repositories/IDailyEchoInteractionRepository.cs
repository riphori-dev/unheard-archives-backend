...
namespace Tywynh.Domain.Repositories
{
    ...
    {
        Task<bool> UserHasInteractedAsync(DateTime echoDate, string? visitorTokenHash, CancellationToken ct = default);
        Task<DailyEchoInteraction?> GetByEchoDateAndUserAsync(DateTime echoDate, string? visitorTokenHash, CancellationToken ct = default);
